"""Tests for ai_context/utils.py — strip_code_fence and call_model routing."""
...
from ai_context.utils import strip_code_fence, call_model
...
class TestCallModelRouting:
    def test_routes_to_litellm_when_model_has_slash(self):
        with patch.dict('os.environ', {'AICONTEXT_MODEL': 'openai/gpt-4o-mini'}, clear=False):
            with patch('ai_context.utils._call_via_litellm', return_value='ok') as mock_litellm:
    ...
    def test_routes_to_anthropic_sdk_when_key_set(self):
        ...
        with patch.dict('os.environ', env, clear=False):
            with patch('ai_context.utils._call_via_anthropic_sdk', return_value='ok') as mock_sdk:
                result = call_model("hello")
    ...
    def test_routes_to_cli_when_no_key(self):
        ...
        with patch.dict('os.environ', env, clear=False):
            ...
            with patch('os.environ.get') as mock_get:
                ...
                with patch('ai_context.utils._call_via_cli', return_value='ok') as mock_cli:
                    result = call_model("hello")
    ...
    def test_litellm_takes_priority_over_api_key(self):
        ...
        with patch.dict('os.environ', env, clear=False):
            with patch('ai_context.utils._call_via_litellm', return_value='ok') as mock_litellm:
                with patch('ai_context.utils._call_via_anthropic_sdk') as mock_sdk:
                    call_model("hello")
    ...
    def test_exits_when_litellm_not_installed(self):
        with patch.dict('sys.modules', {'litellm': None}):
            with pytest.raises(SystemExit):
                from ai_context.utils import _call_via_litellm
